...
| `bidirectional_first_seen_ms` | `int` |        双向流的第一个数据包上的时间戳（以毫秒为单位）        |
| `bidirectional_last_seen_ms`  | `int` |        双向流最后一个数据包上的时间戳（以毫秒为单位）        |
|  `bidirectional_duration_ms`  | `int` |               双向流动持续时间（以毫秒为单位）               |
|    `bidirectional_packets`    | `int` |                       流双向数据包个数                       |
|     `bidirectional_bytes`     | `int` |           流双向字节总数（取决于accounting_mode）            |
|    `src2dst_first_seen_ms`    | `int` |      src2dst方向第一个数据包上的时间戳（以毫秒为单位）       |
|    `src2dst_last_seen_ms`     | `int` |      src2dst方向最后一个数据包的时间戳（以毫秒为单位）       |
|     `src2dst_duration_ms`     | `int` |            src2dst方向流持续时间（以毫秒为单位）             |
|       `src2dst_packets`       | `int` |                    src2dst方向数据包数量                     |
|        `src2dst_bytes`        | `int` |         src2dst方向字节总量（取决于accounting_mode）         |
|    `dst2src_first_seen_ms`    | `int` |       dst2src方向第一个数据包的时间戳（以毫秒为单位）        |
|    `dst2src_last_seen_ms`     | `int` |      dst2src方向最后一个数据包的时间戳（以毫秒为单位）       |
|     `dst2src_duration_ms`     | `int` |             dst2src方向持续时间（以毫秒为单位）              |
|       `dst2src_packets`       | `int` |                    dst2src方向数据包总数                     |
|        `dst2src_bytes`        | `int` |         dst2src方向字节总量（取决于accounting_mode）         |
...
|              特征              |  类型   |                             描述                             |
| :----------------------------: | :-----: | :----------------------------------------------------------: |
|     `bidirectional_min_ps`     |  `int`  |       双向流中最小数据包大小（取决于accounting_mode）        |
|    `bidirectional_mean_ps`     | `float` |       双向流中平均数据包大小（取决于accounting_mode）        |
|   `bidirectional_stddev_ps`    | `float` |   双向流中数据包大小样本标准偏差（取决于accounting_mode）    |
|     `bidirectional_max_ps`     |  `int`  |     双向流中双向最大数据包大小（取决于accounting_mode）      |
|        `src2dst_min_ps`        |  `int`  |    src2dst方向流中最小数据包大小（取决于accounting_mode）    |
|       `src2dst_mean_ps`        | `float` |    src2dst方向流中平均数据包大小（取决于accounting_mode）    |
|      `src2dst_stddev_ps`       | `float` | src2dst方向流中数据包大小样本标准偏差（取决于accounting_mode） |
|        `src2dst_max_ps`        |  `int`  |    src2dst方向流中最大数据包大小（取决于accounting_mode）    |
|        `dst2src_min_ps`        |  `int`  |     dst2src方向流中最小数据包大小（取决于accounts_mode）     |
|       `dst2src_mean_ps`        | `float` |     dst2src方向流中平均数据包大小（取决于accounts_mode）     |
|      `dst2src_stddev_ps`       | `float` | dst2src方向流中数据包大小样本标准偏差（取决于accounting_mode） |
|        `dst2src_max_ps`        |  `int`  |    dst2src方向流中最大数据包大小（取决于accounting_mode）    |
|  `bidirectional_min_piat_ms`   |  `int`  |                 双向流中数据包的最小时间间隔                 |
|  `bidirectional_mean_piat_ms`  | `float` |               双向流中数据包之间的平均时间间隔               |
| `bidirectional_stddev_piat_ms` | `float` |                  双向流中数据包间隔的标准差                  |
|  `bidirectional_max_piat_ms`   |  `int`  |               双向流中数据包之间最大的时间间隔               |
|     `src2dst_min_piat_ms`      |  `int`  |             src2dst方向流中数据之间最小时间间隔              |
|     `src2dst_mean_piat_ms`     | `float` |           src2dst方向流中数据包之间的平均间隔时间            |
|    `src2dst_stddev_piat_ms`    | `float` |         src2dst方向流中数据包之间的间隔时间标准偏差          |
|     `src2dst_max_piat_ms`      |  `int`  |          src2dst方向流中数据包之间时间间隔的最大值           |
|     `dst2src_min_piat_ms`      |  `int`  |          dst2src方向流中数据包之间的时间间隔最小值           |
|     `dst2src_mean_piat_ms`     | `float` |            dst2src方向流中数据之间的平均时间间隔             |
|    `dst2src_stddev_piat_ms`    | `float` |          dst2src方向流中数据包之间时间间隔的标准差           |
|     `dst2src_max_piat_ms`      |  `int`  |           dst2src方向流中数据之间时间间隔的最大值            |
